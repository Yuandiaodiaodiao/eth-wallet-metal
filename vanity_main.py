...
def main(batch_size: int = 384*8, nibble: int = 0x0, nibble_count: int = 6, max_batches: Optional[int] = None, steps_per_thread: int = 256*16, prefix_hex: Optional[str] = "123456") -> None:
    ...
    job_0 = engine.encode_and_commit_walk_compact(privs_0, steps_per_thread=steps_per_thread, nibble=nibble, nibble_count=nibble_count, prefix_hex=prefix_hex)
    
    t_gen1 = time.perf_counter()
    privs_1 = generate_valid_privkeys(batch_size, steps_per_thread, 128)
    gen_1_sec = time.perf_counter() - t_gen1
    job_1 = engine.encode_and_commit_walk_compact(privs_1, steps_per_thread=steps_per_thread, nibble=nibble, nibble_count=nibble_count, prefix_hex=prefix_hex)
    ...
    while True:
        ...
        if indices:
            ...
            verify_job = engine.encode_and_commit_walk_compact([k_bytes], steps_per_thread=1, nibble=0x0, nibble_count=0, prefix_hex=None)
            v_indices, _ = engine.wait_and_collect_compact(verify_job)
            print(f'walk indices: {v_indices}')
            
            print("\nTesting compact:")
            verify_job_correct = engine.encode_and_commit_compact([k_bytes], nibble=0x0, nibble_count=0, prefix_hex=None)
        ...
        jobs[oldest_idx] = engine.encode_and_commit_walk_compact(privs[oldest_idx], steps_per_thread=steps_per_thread, nibble=nibble, nibble_count=nibble_count, prefix_hex=prefix_hex)
